...
sidebar <- dashboardSidebar(
  hr(),
  sidebarMenu(id="tabs",
              menuItem("Network by Date", tabName="network-date", icon=icon("calendar")),
              menuItem("Network by ID", tabName = "network-id", icon=icon("id-badge"), selected=TRUE)
  )
)
...
body <- dashboardBody(
  tabItems(
    tabItem(tabName = "network-date",
            ...
            visNetworkOutput("mynetwork.date")
            
            
    ),
    tabItem(tabName = "network-id",
