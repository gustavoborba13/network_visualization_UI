...
sidebar <- dashboardSidebar(
  hr(),
  sidebarMenu(id="tabs",
              menuItem("Network", tabName="network-date", icon=icon("connectdevelop")) #sidebar with vis title and icon
  )
)

#body
body <- dashboardBody(                                        #dashboard body, include the visualization clicked on sidebar
  tabItems(
    tabItem(tabName = "network-date",
            ...
            span(textOutput(outputId = "id"), style = "color:red"),    #textOutput stating the ID or error in RED
            visNetworkOutput("mynetwork"),                             #visNetworkOutput based on ID typed and date range
